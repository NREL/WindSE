...
class Optimizer(object):
    ...
    def Optimize(self):
        ...
        if "SNOPT" in self.opt_routine or "OM_SLSQP" in self.opt_routine:
            if "layout" in self.control_types:
                m_opt=opt_function(self.Jhat, method="Custom", options = {"disp": True}, constraints = self.dist_constraint, bounds = self.bounds, callback = self.OptPrintFunction, algorithm=om_wrapper, opt_routine=self.opt_routine)
            else:
                m_opt=opt_function(self.Jhat, method="Custom", options = {"disp": True}, bounds = self.bounds, callback = self.OptPrintFunction, algorithm=om_wrapper, opt_routine=self.opt_routine)
        else:
            if "layout" in self.control_types:
                m_opt=opt_function(self.Jhat, method=self.opt_routine, options = {"disp": True}, constraints = self.dist_constraint, bounds = self.bounds, callback = self.OptPrintFunction)
            else:
                m_opt=opt_function(self.Jhat, method=self.opt_routine, options = {"disp": True}, bounds = self.bounds, callback = self.OptPrintFunction)
