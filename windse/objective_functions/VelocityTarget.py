...
def objective(solver, inflow_angle = 0.0, first_call=False, **kwargs):
    ...
    target = float(kwargs.pop("target"))
    ...
    if V <= 1e-10:
        ...
        u_mag = sqrt(solver.problem.u_k[0]**2.0+solver.problem.u_k[1]**2.0+solver.problem.u_k[2]**2.0)

        ### Evaluate objective ###
        J = assemble(plane_marker*(u_mag-target)**2.0*dx)/V
    return J
